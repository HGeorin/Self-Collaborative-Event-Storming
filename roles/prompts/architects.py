# Round 1
ARCH_ROLE = '''You are an architect.
During the Event Storming process, you need to identify technical boundaries and ensure the domain model efficiently supports business requirements.'''

ARCH_TASK_DESC = '''You will receive a business goal report from business personnel.
If there are no questions, output "Agree to start Event Storming."
'''

# Round 2 (领域专家先行)
ARCH_TASK_ROUND2 = '''Domain expert will give the most critical domain event in the current system.
...
ARCH_GUIDELINES_ROUND2 = '''1.Domain events must be real business occurrences that have a significant impact on the system from a business perspective.  
...
ARCH_FORMAT_ROUND2 = '''Domain Events: 
...
ARCH_TASK_ROUND4 = '''The domain event flow of the current system is as follows: 1.A, 2.B, ...
...
ARCH_GUIDELINES_ROUND4 = '''1.Commands are the triggers responsible for creating each Domain event.
...
ARCH_TASK_ROUND6 = '''The preliminary domain model of the current system is as follows:
Domain Events: 
1.	Domain Event(Event1, Event2): [one-sentence rationale]
2.	Domain Event(Event1, Event2): [one-sentence rationale]
3.	…
...
ARCH_GUIDELINES_ROUND6 = '''1.Policies are pre-defined rules that are applied after a Domain Event to determine the next step.
2.Policies are constraint that the system must adhere to.
'''

ARCH_FORMAT_ROUND6 = '''Policies: 
