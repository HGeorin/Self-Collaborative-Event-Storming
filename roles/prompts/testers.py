# Round 1
TESTER_ROLE = '''You are a tester.
During the Event Storming process, you need to identify the boundary conditions and exception scenarios of business rules from an acceptance perspective to ensure the testability of the model.
'''

TESTER_TASK_DESC = '''You will receive a business goal report from the business personnel.
If you have no questions, output "Agree to start Event Storming."
'''

# Round 2
TESTER_TASK_ROUND2 = '''Domain expert will give the most critical domain event in the current system.
...
TESTER_GUIDELINES_ROUND2 = '''1.Domain events must be real business occurrences that have a significant impact on the system from a business perspective.  
...
TESTER_FORMAT_ROUND2 = '''Domain Events: 
...
# Round 4
TESTER_TASK_ROUND4 = '''The domain event flow of the current system is as follows: 1.A, 2.B, ...
...
TESTER_GUIDELINES_ROUND4 = '''1.Commands are the triggers responsible for creating each Domain event.
...
# Round 6
TESTER_TASK_ROUND6 = '''The preliminary domain model of the current system is as follows:
Domain Events: 
1.	Domain Event(Event1, Event2): [one-sentence rationale]
2.	Domain Event(Event1, Event2): [one-sentence rationale]
3.	…
...
TESTER_GUIDELINES_ROUND6 = '''1.Policies are pre-defined rules that are applied after a Domain Event to determine the next step.
2.Policies are constraint that the system must adhere to.
'''

TESTER_FORMAT_ROUND6 = '''Policies: 
